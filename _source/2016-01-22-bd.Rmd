...
dados_txt <- d_cjpg_final2 %>% 
  inner_join(select(d_cjpg_one, n_processo, 
                    txt, magistrado, vara, 
                    data_disponibilizacao), 'n_processo') %>% 
  select(-antiguidade) %>% 
  select(n_processo, cod_sentenca, foro, vara, magistrado, digital,
         tipo_vara, data_disponibilizacao,
         adv_reu, empresa, tipo_dano, resultado, valor_acao,
         resultado_vl, tempo, serasa, terceiro, consumo, 
         gratuidade, txt)
...
saveRDS(dados_txt, file = 'dados_txt.rds')
save(dados, file = 'dados.rda')
save(dados_txt, file = 'dados_txt.rda')
openxlsx::write.xlsx(dados, 'dados.xlsx')
openxlsx::write.xlsx(dados_txt, 'dados_txt.xlsx')
```
